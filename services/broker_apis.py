...
def to_decimal(x, default: Decimal = Decimal("0")) -> Decimal:
    """
    Safely convert any value to Decimal.
    Works with strings like '352.389712783', numbers, or None.
...
class AlpacaAPI(BrokerAPI):
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"Successfully authenticated with Alpaca "
                    f"{'Paper Trading' if self.paper_trading else 'Live Trading'}"
                )
                return True
            else:
    ...
    def get_positions(self) -> List[Dict]:
        ...
        for pos in positions:
            qty_dec = to_decimal(pos.get("qty"))
            cleaned_positions.append(
                {
                    "symbol": pos.get("symbol"),
                    # Keep quantity as string – supports fractional shares like "352.389712783"
                    "quantity": pos.get("qty"),
                    # Alpaca returns these as strings; keep them as-is and let callers decide how to parse
                    "market_value": pos.get("market_value"),
                    "cost_basis": pos.get("cost_basis"),
                    "unrealized_pnl": pos.get("unrealized_pnl", "0"),
                    # Determine side based on sign of qty (works with fractional)
                    "side": "long" if qty_dec > 0 else "short" if qty_dec < 0 else "flat",
                }
            )
...
class BrokerManager:
    ...
    def _get_broker_from_env(self) -> str:
        ...
        try:
            session = get_session()
            trading_mode = (
                session.query(GlobalEnvVar)
                .filter(GlobalEnvVar.name == "TRADING_MODE")
                .first()
            )
            active_broker = (
                session.query(GlobalEnvVar)
                .filter(GlobalEnvVar.name == "ACTIVE_BROKER")
                .first()
            )
    ...
    def place_order(
        ...
        return self.get_active_broker().place_order(
    ...
    def get_historical_data(
        ...
        return self.get_active_broker().get_historical_data(
